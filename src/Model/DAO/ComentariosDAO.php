...
namespace src\Model\DAO {
	...
	class ComentariosDAO extends MySQL
	{
		...
		public function SelectAllByIncident($IDIncidente)
		{
			$STMT = parent::PREPARE('SELECT 
												a.Usuario Usuario,
												a.PK_IDComentarios PK_IDComentarios,
												a.FechaComentario FechaComentario,
												a.DescripcionComentario DescripcionComentario,
												COALESCE(b.Nombre, c.NombreEmpresa) AS Nombre
											FROM Comentarios a
											LEFT JOIN Cliente b ON a.Usuario = b.PK_IDCliente
											LEFT JOIN Proveedor c ON  a.Usuario =  c.PK_IDProveedor
											WHERE a.Incidente = ?
											ORDER BY a.FechaComentario ASC;;');
